...
export default function AuEconomyPage() {
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/economy?country=au&set=core&start=${fetchStart}`,
          { cache: "no-store" }
        );
  ...
  return (
    <div className="w-full space-y-4">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-white">Australia Economy Snapshot</h1>
          <p className="mt-1 text-sm text-slate-400">
            Trial snapshot built from calendar history for key Australia growth,
            inflation, labour, policy and positioning indicators.
          </p>
        </div>
