...
/**
 * This is the ACTUAL visible range the user selected.
 */
...
/**
 * This is the fetch range. We fetch some extra history so
 * trend calculations like 3m vs 6m still have enough lookback.
 */
...
export default function UkEconomyPage() {
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/economy?country=gb&set=core&start=${fetchStart}`,
          { cache: "no-store" }
        );
  ...
  return (
    <div className="w-full space-y-4">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-white">UK Economy Snapshot</h1>
          <p className="mt-1 text-sm text-slate-400">
            Trial snapshot built from calendar history for key UK growth, inflation,
            labour and policy indicators.
          </p>
        </div>
