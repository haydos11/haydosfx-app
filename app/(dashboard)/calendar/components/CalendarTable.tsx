...
function extractPower(row?: Multiplierish | null): number | null {
  const raw = row?.multiplier ?? row?.scale ?? row?.unit_multiplier ?? row?.multiplier_power ?? null;
...
/* ---------------- micro-UI ---------------- */
function impactBadge(imp?: number | null): { label: "" | "High" | "Moderate" | "Low"; cls: string } {
...
function arrow(actual?: number | null, forecast?: number | null): { glyph: "" | "↑" | "↓"; cls: string } {
...
function resolveEventId(row: CalendarEventRow): number | null {
  ...
  const maybe = r.event_id ?? r.eventCode ?? r.event ?? r.event_code ?? r.eventId ?? null;
  const n =
    typeof maybe === "number" ? maybe : typeof maybe === "string" ? Number(maybe) : NaN;
...
export default function CalendarTable({
  ...
  return (
    <div className="mt-3 w-full overflow-hidden rounded-3xl border border-white/10 ring-1 ring-white/5 bg-[#0b0b0b]/70 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.6)]">
      ...
      <div className="max-h-[72vh] overflow-y-auto [scrollbar-gutter:stable] touch-pan-y">
        {/* Horizontal scroll container (mobile swipe) */}
        <div
          className="w-full overflow-x-auto [-webkit-overflow-scrolling:touch] touch-pan-x overscroll-x-contain"
          style={{ WebkitOverflowScrolling: "touch" }}
        >
          {/* IMPORTANT: remove table-fixed so overflow can actually happen */}
          <table className="min-w-[760px] w-full text-[12.5px] leading-tight">
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className={tdBase}>
                            {isNum(e.actual) ? (
                              <div className="flex items-center gap-1 leading-none">
                                ...
                                <span className="tabular-nums text-slate-100">{fmtValue(e.actual, unit, e)}</span>
                              </div>
                            ) : (
                          ...
                          <td className={`${tdBase} text-center`}>
                            <span onClick={(ev) => ev.stopPropagation()} className="inline-flex leading-none scale-90 origin-center">
                              <AnalyzeButton
                                compact
